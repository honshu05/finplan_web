...
import { runSimulation, optimizeRebalancingTargets } from './utils/finance';
import type { SimulationInputs, SimulationRecord, OptimizedStrategy } from './utils/finance';
...
export default function App() {
  ...
  const [activePanel, setActivePanel] = useState<string | null>('demographics');
  ...
  const simulationData = useMemo(() => {
    ...
    const raw = runSimulation(mktVal, effectiveInputs, illF, illA, enableRebalance);
    const zeroIndex = raw.records.findIndex(r => r.Balance <= 0);
    if (zeroIndex !== -1) {
      return { ...raw, records: raw.records.slice(0, zeroIndex + 2) };
    }
    return raw;
  }, [effectiveInputs, stressParams, enableRebalance, enableStress]);
  ...
  const taxSavingsReport = useMemo(() => {
    ...
    // Simulate Lump Sum
    const lsResult = runSimulation(mktVal, lumpSumInputs, illF, illA, enableRebalance);
  ...
  }, [effectiveInputs, stressParams, enableStress, enableRebalance, simulationData]);
        ...
        <header className="h-[50px] border-b border-white/5 bg-[#030508] flex items-center justify-between px-8 z-50 shrink-0">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-sm font-black tracking-tighter uppercase whitespace-nowrap">
              FinPlan <span className="text-accent underline decoration-accent/20">Suite V0.4</span>
            </h1>
              ...
              <div className="flex-1 min-h-0 glass p-8 shadow-3xl border-white/5 flex flex-col">
                ...
                <div className="flex items-center justify-between border-b border-white/5 pb-4 mb-6 shrink-0">
                  <span className="text-[12px] font-black uppercase tracking-[0.3em] text-accent/80">
                    {viewMode === 'rebalanceReport' ? 'Milestone Staged Rebalancing Strategy' : (viewMode === 'decumulation' ? 'Post-Retirement Decumulation Flows' : 'Asset Lifespan Trace')}
                  </span>
                      ...
                      <table className="w-full text-left border-collapse text-[11px]">
                        <thead className="uppercase font-black tracking-[0.25em] sticky top-0 bg-[#0F1219] z-10">
                          ...
                          <tr className="text-[8px] border-b border-white/3">
                            <th colSpan={8} className="px-3 py-1 text-white/20 border-r border-white/5">Portfolio & Cashflow</th>
                            <th colSpan={5} className="px-3 py-1 text-[#FF6B35]/60">▸ Expense Funds (post-retirement sourcing)</th>
                          </tr>
                          {/* Column header row */}
